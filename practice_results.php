...
<div class="container mt-5">
    <?php display_flash_message('flash_message', 'flash_message_type'); ?> <div class="card shadow-sm">
        <div class="card-header bg-light">
            <h2 class="text-center mb-0"><?php echo $page_title; ?></h2> </div>
        <div class="card-body p-4">
            <div class="text-center mb-4">
                <h3>আপনি পেয়েছেন: <strong class="text-primary"><?php echo number_format($total_score_practice, 0); ?> / <?php echo $total_questions_practice; ?></strong></h3> </div>
            
            <div id="quizResultChartContainer" class="mb-4"> <canvas id="quizResultChart"></canvas> </div>

            <hr> <div id="printableArea"> <div class="print-header">অনুশীলন কুইজ: <?php echo htmlspecialchars($category_name); ?></div> <h3 class="mt-4 mb-3">উত্তর পর্যালোচনা</h3> <div class="answer-review"> <?php if (!empty($review_questions_practice)): ?>
                        <?php foreach ($review_questions_practice as $index => $question): ?>
                        <div class="card mb-3">
                            <div class="card-header">
                                <strong>প্রশ্ন <?php echo $index + 1; ?>:</strong> <?php echo nl2br(htmlspecialchars($question['question_text'])); ?> </div>
                            <div class="card-body">
                                <?php if (!empty($question['image_url'])): ?>
                                    <div class="mb-2 text-center">
                                        <img src="<?php echo $base_url . escape_html($question['image_url']); ?>" alt="প্রশ্ন সম্পর্কিত ছবি" class="img-fluid question-image-review"> </div>
                                <?php endif; ?>
                                <ul class="list-group list-group-flush">
                                    ...
                                    <?php foreach ($question['options_list'] as $option): ?>
                                        ...
                                        ?>
                                    <li class="<?php echo $option_class; ?>"> <?php echo htmlspecialchars($option['text']); ?> <?php echo $option_label; ?> </li>
                                    <?php endforeach; ?>
                                </ul>
                                <?php if ($question['user_selected_option_id'] === null) : ?> <p class="mt-2 mb-0 text-warning print-header-message">আপনি এই প্রশ্নের উত্তর দেননি।</p> <?php endif; ?>

                                <?php if (!empty($question['explanation'])): ?> <div class="mt-3 p-2 bg-light border rounded">
                                    <strong>ব্যাখ্যা:</strong> <?php echo nl2br(htmlspecialchars($question['explanation'])); ?> </div>
                                <?php endif; ?>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="alert alert-info print-header-message">এই ফলাফলের জন্য উত্তর পর্যালোচনা উপলব্ধ নয়।</p> <?php endif; ?>
                </div>
            </div>
            <div class="text-center mt-4">
                <button onclick="window.print()" class="btn btn-outline-primary">উত্তরপত্র প্রিন্ট করুন</button> <a href="categories.php" class="btn btn-secondary">সকল ক্যাটাগরি দেখুন</a> <?php if ($category_id > 0): ?> <a href="practice_quiz.php?category_id=<?php echo $category_id; ?>" class="btn btn-info">আবার অনুশীলন করুন</a> <?php endif; ?>
            </div>
        </div>
    </div>
</div>
