...
try {
    // 1. Fetch the original quiz details
    ...
    // 2. Create the new (duplicate) quiz entry
    $new_quiz_title = "ডুপ্লিকেট - " . $original_quiz_data['title'];
    $new_quiz_status = 'draft'; // Default status for duplicated quiz
    $new_live_start = null;     // Clear live dates for duplicated quiz
    $new_live_end = null;
    ...
    // 3. Fetch questions from the original quiz
    $sql_original_questions = "SELECT * FROM questions WHERE quiz_id = ? ORDER BY order_number ASC, id ASC";
    ...
    while ($original_question_data = $result_original_questions->fetch_assoc()) {
        ...
        if (!empty($original_question_data['image_url'])) {
            ...
            if (file_exists($original_image_full_path)) {
                ...
                // Create a more unique name for the duplicated image
        ...
        // 3b. Insert the new question
        $sql_insert_new_question = "INSERT INTO questions (quiz_id, question_text, image_url, explanation, order_number) VALUES (?, ?, ?, ?, ?)";
        ...
        $stmt_new_question->bind_param(
            ...
            $new_question_image_url, // Use duplicated image URL or null
        ...
        // 4. Fetch options for the original question
        ...
        while ($original_option_data = $result_original_options->fetch_assoc()) {
            ...
            if (!$stmt_new_option->execute()) {
                throw new Exception("নতুন অপশন সংরক্ষণ করতে সমস্যা: " . $stmt_new_option->error);
            }
